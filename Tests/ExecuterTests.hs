...
secondary = let sent = [0x22,0x20,0x00]
                expect = [Match a | a <- [0x62,0x20,0x0,0x40,0xf,0xff,0xf,0x80,0x40,0x2,0xf,0x80,0x40,0x27,0xf,0x80,0x40,0x2b,0x4c,0x80,0x41,0x88,0xe,0x80,0x41,0xa6,0xf,0x93,0x7,0x1f,0xf]] in
            DiagScript [ScriptTestCase (TestCase "Secondary Error Memory"
                            (DiagScriptMsg Nothing Nothing sent)
                            (ExpectedMsg   Nothing Nothing (ExpectedPayload [expect]))
...
example :: FilePath -> IO (Either ParseError DiagScript)
example f = do
  s <- readFile f
...
examples = do scripts <- allSkriptFiles >>= mapM example
              let rightScripts = rights scripts
              files   <- allSkriptFiles
              return $ zip files rightScripts
